...
public class DarkTheme extends AbstractTheme {
    ...
    @Override public void drawHeader(Panel panel, int x, int y, int w, int h, boolean hovered) {
        drawRect(x - 2, y - 2, x + w + 2, y, new Color(30, 30, 30, 240).getRGB());
        drawRect(x, y, x + w, y + h - 2, new Color(40, 40, 40, 225).getRGB());
        if (panel.isOpen())
            Gui.drawRect(x, y + h - 2, x + w, y + h, new Color(40, 40, 40, 225).getRGB());
        FontRender.drawStringWithShadow(panel.category.name(), x + 3, y + 4, Screen.color.getRGB());
    ...
    @Override public void drawPanelOutline(Panel panel, int x, int y, int w, int h, boolean hovered) {
        ...
        drawRect(x - 2, y, x, y + localHeight, new Color(40, 40, 40, 225).getRGB());
        drawRect(x + w, y, x + w + 2, y + localHeight, new Color(40, 40, 40, 225).getRGB());
        drawRect(x - 2, y + localHeight, x + w + 2, y + localHeight + 2, new Color(40, 40, 40, 225).getRGB());
    }
